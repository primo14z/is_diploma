...
@csrf_exempt
def narocila_K_Moja(request):
	uporabnik = request.user.id
	
	narocilo=Narocilo_K.objects.filter(prodajalec_K_id = uporabnik).order_by('datum_K_N')
	print(narocilo)
...
@csrf_exempt
def narocila_K_obdelana(request):
